...
const http = axios.create({
  baseURL: import.meta.env.DEV ? '/zk' : 'https://ly.pcleartv.cn/liubangguli/',
})
const isDev = import.meta.env.MODE === 'development'
const project = getExtConfig()
http.interceptors.request.use(
  (config) => {
    // console.log('%c [ query ]-9', 'font-size:13px; background:pink; color:#bf2c9f;', config)
    if (config.custom?.auth && window.localStorage.getItem('token')) {
